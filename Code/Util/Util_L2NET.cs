...
namespace L2Robot
{
    public static partial class Util
    {
        ...
        public static void Setup_Threads()
        {
            ...
            //Globals.oog_loginthread = new Thread(new ThreadStart(LoginServer.OOG_Login));
        ...
        {
            //Globals.l2net_home.Add_Debug("Killing all network connections");
        ...
        {
            //Globals.l2net_home.Add_Debug("Killing all threads");
            ...
            try
            {
                for (int i = 0; i < Globals.gamethreads.Count; i++)
                {
                    if (Globals.gamethreads[i] != null)
                    {
                        Globals.gamethreads[i].readthread.Abort();
                    }
                }
            }
            catch//(Exception e)
        ...
        {
            //Globals.l2net_home.Add_Debug("*******************************");
            //Globals.l2net_home.Add_Debug("Handle count: " + System.Diagnostics.Process.GetCurrentProcess().HandleCount.ToString());
            //Globals.l2net_home.Add_Debug("Memory usage / allocated: " + GC.GetTotalMemory(false).ToString() + " / " + Environment.WorkingSet.ToString());
            //Globals.l2net_home.Add_Debug("Cleaning up memory... this may take a few moments");
            //VoicePlayer.PlaySound(7);
            GC.Collect();
            //Globals.l2net_home.Add_Debug("Clean up complete!");
            //Globals.l2net_home.Add_Debug("Handle count: " + System.Diagnostics.Process.GetCurrentProcess().HandleCount.ToString());
            //Globals.l2net_home.Add_Debug("Memory usage / allocated: " + GC.GetTotalMemory(true).ToString() + " / " + Environment.WorkingSet.ToString());
            //Globals.l2net_home.Add_Debug("*******************************");
        }
        ...
        public static void SayToClient(int index, string end)
        {
            ...
            ByteBuffer bbuff;
            if (this.gamedata.Chron >= Chronicle.CT2_6)
            {
            ...
            this.gamedata.SendToClient(bbuff);
        }
#endif
