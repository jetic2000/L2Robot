namespace L2Robot
{
    public class MixedPackets
    {
        ...
        public MixedPackets(GameData gamedata, int Seed)
        {
            _seed = Seed;
        ...
        {
            if (this.gamedata.Chron >= Chronicle.CT2_5 && Packet[2] == (byte)PClient.EnterWorld)// || (this.gamedata.Chron >= Chronicle.CT3_0 && Packet[2] == (byte)PClient.Action))
            {
            ...
            else
            {
                ...
                else if (this.gamedata.Chron >= Chronicle.CT2_5)//If not expacket and Freya
                {
                    Packet[2] = (byte)GetIndexOf(Packet[2], PacketIDs);
                }
                if (this.gamedata.Chron <= Chronicle.CT2_4)
                {
                    Packet[2] = (byte)GetIndexOf(Packet[2], PacketIDs);
                }
            }
        ...
        public void Encrypt0(byte[] Packet)
        {
            try
            {
                if (this.gamedata.Chron >= Chronicle.CT2_5 && Packet[0] == (byte)PClient.EnterWorld)// || (this.gamedata.Chron >= Chronicle.CT3_0 && Packet[0] == (byte)PClient.Action))
                {
                ...
                else
                {
                    ...
                    else if (this.gamedata.Chron >= Chronicle.CT2_5)//freya
                    {
        ...
        public void Encrypt0(ByteBuffer Packet)
        {
            try
            {
                if (this.gamedata.Chron >= Chronicle.CT2_5 && Packet.GetByte(0) == (byte)PClient.EnterWorld)//|| (this.gamedata.Chron >= Chronicle.CT3_0 && Packet.GetByte(0) == (byte)PClient.Action))
                {
        ...
        public void Decrypt(byte[] Packet)
        {
            ...
            else
            {
                ...
                else if (this.gamedata.Chron >= Chronicle.CT2_5)
                {
                    Packet[2] = PacketIDs[Packet[2]];
                }
            }
