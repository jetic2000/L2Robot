    ...
    public class NPCInfo : Object_Base
    {
        ...
        public void Load(ByteBuffer buff)
        {
            ...
            */
            ...
            //Globals.l2net_home.Add_Debug("nametemp: " + nametemp + "Name: " + Name + "Title: " + Title, Globals.Red, TextType.ALL);
            ...
            {
#if DEBUG
                //Globals.l2net_home.Add_Debug("More than 30 abnormal effects, probably an error... Count: " + abn_count, Globals.Yellow, TextType.BOT);
#endif
                abn_count = 10;
            }

            //Globals.l2net_home.Add_Debug("count " + abn_count, Globals.Yellow, TextType.BOT);

            AbnEffects.Clear();
            for (uint i = 0; i < (uint)abn_count; i++)
            {
                uint tmpabneff = buff.ReadUInt32();
                //Globals.l2net_home.Add_Debug("Adding abnormal vfx to npc : " + tmpabneff.ToString("X2") + " to : " + Name, Globals.Yellow, TextType.BOT);
        ...
        public void Update(ByteBuffer buff)
        {
            ...
            {
                uint StatToUpdate = buff.ReadUInt32();
                switch (StatToUpdate)
                {
                    case 0x01://level
